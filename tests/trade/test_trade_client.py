...
class TestApi(unittest.TestCase):
    def test_api(self):
        ...
        res = trade_client.trade_instrument.get_trade_instrument_detail("913256135")
        if res.status_code == 200:
            print(res.json())
        res = trade_client.trade_instrument.get_trade_instrument_detail("913256409")
        if res.status_code == 200:
            print(res.json())
        res = trade_client.trade_instrument.get_trade_instrument_detail("913244615")
        if res.status_code == 200:
            print(res.json())
        res = trade_client.trade_instrument.get_trade_security_detail("SPX", Markets.US.name, "OPTION",
                                                                      InstrumentType.CALL_OPTION.name, "3400", "2024-12-20")
        if res.status_code == 200:
            print(res.json())
        res = trade_client.trade_instrument.get_tradeable_instruments()
        if res.status_code == 200:
        ...
        res = trade_client.order.place_order(stock_order['account_id'], **stock_order['stock_order'])
        trade_client.order.remove_custom_headers()
        if res.status_code == 200:
            print('place order res:', res.json())
        res = trade_client.order.replace_order(stock_order['account_id'], **stock_order['stock_order'])
        if res.status_code == 200:
