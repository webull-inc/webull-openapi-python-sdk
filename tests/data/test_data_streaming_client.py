...
from webull.data.internal.quotes_client import LOG_INFO
...
class TestDataStreamingClient(unittest.TestCase):

    def test_data_streaming_client(self):
        def my_connect_success_func(client, api_client, quotes_session_id):
            client.easy_log(LOG_INFO, "connect success with session_id:%s", quotes_session_id)
            # subscribe
            symbols = ['00700']
        ...
        def my_quotes_message_func(client, topic, quotes):
            client.easy_log(LOG_INFO, "receive message: topic:%s, quotes:%s", topic, quotes)

        def my_subscribe_success_func(client, api_client, quotes_session_id):
            client.easy_log(LOG_INFO, "subscribe success with session_id:%s", quotes_session_id)
